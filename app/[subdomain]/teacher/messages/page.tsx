...
const TeacherMessagesList = () => {
  const [messages, setMessages] = useState<any>([
    // { id: 1, sender: 'Admin', content: 'Welcome to the new school year!', replies: [] },
    { id: 1, sender: 'Teacher', content: 'Please submit your assignments by Friday.', replies: [] },
  ...
  const handleSendMessage = () => {
    setMessages([...messages, { id: messages.length + 1, sender: 'Teacher', content: newMessage, replies: [] }]);
    setNewMessage('');
  };
  ...
  const handleReplyMessage = (id: any) => {
    const updatedMessages = messages.map((msg: { id: any; replies: any; }) => {
      if (msg.id === id) {
        return { ...msg, replies: [...msg.replies, { sender: 'Teacher', content: replyMessage }] };
      }
      return msg;
    });
