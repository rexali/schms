...
export default function AddScoreSheet(props: any) {
    ...
    const addScoreSheet = async () => {
        ...
        const markSheetResponse = await fetch('/api/marksheets', {
            mode: 'cors',
            method: "POST",
                ...
                <table className="table table-bordered table-striped">
                    <thead className="thead-dark">
                        <tr>
                            ...
                            {
                                scoreSheet.students[0].scores.map(score => <th key={score.week} className="text-center">M {score.week}</th>)
                            }
                                ...
                                return index===0 ? (
                                    <tr key={index}>
                                        ...
                                        <td className="text-align-middle" colSpan={2}>
                                            <input type="text" className="d-inline-block text-end" size={10} value={"Max.Score"} disabled />
                                            <input type="text" className="d-inline-block align-self-center" size={10} value={student.name} onChange={(ev) => setStudent(ev, student.id)} />
                                        </td>
                                        {
                                            student.scores.map(score => (
                                                <td key={score.week}>
                                                    <input type="text" className="d-inline-block" size={5} value={score.maxMark} name={`maxMark${score.week}`} id="maxMark" onChange={(ev) => setStudentMaxScore(ev, student.id, score.week)} />
                                                    <input type="text" className="d-inline-block" size={5} value={score.score} name={`score${score.week}`} id="score" onChange={(ev) => setStudentScore(ev, student.id, score.week)} />
                                                </td>
                                            ))
                                        }
