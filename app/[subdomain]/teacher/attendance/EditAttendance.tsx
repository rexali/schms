...
const EditAttendance = (props: any) => {
    ...
    const handleUpdateSubmit = async () => {
        ...
        const totalAttendance = {
            weeklyTotals: attendance.students.map(student => ({
            ...
            dailyTotals: ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'].map((day, index) => ({
                ...
                percentage: calculatePercentage(calculateDailyTotal(index, 0) + calculateDailyTotal(index, 1), attendance.students.length)
            })),
            overallTotal: {
                ...
                percentage: calculatePercentage(calculateTotalAttendance(0) + calculateTotalAttendance(1), attendance.students.length)
            }
        };
        console.log('Submitted attendance:', { ...attendance, ...totalAttendance });
        const finalAttendance = { ...attendance, ...totalAttendance, user: userId }
        const attendanceResponse = await fetch("/api/attendance/" + props.attendanceId, {
            method: "PATCH",
            mode: "cors",
    ...
    return (
        <div className="container mt-5">
            <h2 className='d-flex justify-content-between'>Edit weekly attendance <button className='btn btn-success' onClick={() => props.setEdit(false)}>close</button></h2><br />
